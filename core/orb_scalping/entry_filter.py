"""
QuantOS — ORB entry filter (docs/ORB_ENTRY_FILTER_METHODOLOGY.md)
────────────────────────────────────────────────────────────────
Gates candidate 18's entries on the two conditions
docs/ORB_CONDITION_MINING_RESULTS.md found informative, replicated twice
(2026-09-03 and 2026-09-21, each with fresh trades folded in): NIFTY only
on Monday/Friday, BankNifty only on a >0.3% gap day. Exactly the mined
pairing, applied to its own index only — see the methodology doc's Scope
section for why NIFTY does not also get the gap filter and vice versa
(both cross-applications were tested in the mining exercise and failed).

Pure functions only, no I/O, no config. The live script decides WHETHER
to call these (only the filtered variant does; unfiltered candidate 18
never calls this module at all) and supplies whatever data each predicate
needs.
"""
