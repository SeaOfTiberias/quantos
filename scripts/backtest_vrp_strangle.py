#!/usr/bin/env python3
"""
QuantOS — VRP Phase 2+3: Short-Strangle Backtest Driver
──────────────────────────────────────────────────────────
...
core/options/vrp/strikes.py), then computes pooled per-trade P&L from real
recorded prices (Phase 3, core/options/vrp/simulator.py). GROSS, pre-cost
result only — see simulator.py's module docstring for why this isn't a
go/no-go verdict yet; Phase 4 (transaction costs) is separate follow-up work.
...
def main() -> int:
    ...
    print("\n-- VRP Short Strangle -- GROSS, pre-cost -----------------")
    ...
    print(f"Delta-fallback strikes : {fallback_calls} calls / {fallback_puts} puts of {len(trades)}")

    if args.out:
        _write_report(args.out, stats, args.start, args.end, fallback_calls, fallback_puts, len(trades))
        print(f"\nWrote {args.out}")

    return 0


def _write_report(out: Path, stats, start: date, end: date, fallback_calls: int,
    ...
    lines = [
        ...
        "",
        "**This is a gross, pre-transaction-cost read only. Phase 4 (cost model) has not "
        "run. Do not treat this as a go/no-go verdict.**",
        "",
