<#
.SYNOPSIS
    Push a universe file to the QuantOS VM and restart the agent so it
    re-reads it.

.DESCRIPTION
    Bundles the two-step ops chore (scp + systemctl restart) documented
    in docs/DailyRunbook.md. Runs from the laptop (Windows PowerShell).

    Since 2026-07-16 the universe is agent/universe_nifty500.txt, generated by
    scripts/build_universe.py from an NSE constituent CSV. It serves BOTH Stage A
    discovery (scanner.universe_file) and the regime advance/decline sample
    (regime.breadth_universe_file). This is therefore no longer a daily chore:
    regenerate and push at NSE's semi-annual rebalance, not on a whim — the
    universe has to hold still for a Darvas track record to mean anything, and
    editing it also moves the regime gate.
...
param(
    ...
    [string]$LocalFile  = (Join-Path $PSScriptRoot "..\agent\universe_nifty500.txt"),
    [string]$RemotePath = "/home/ubuntu/quantos/agent/universe_nifty500.txt",
    [switch]$Tail,
    [switch]$NoRestart
)
